    ...
    internal sealed class VideoMetadataService {
        ...
        public string NormalizeYouTubeUrl(string url) {
            if (string.IsNullOrWhiteSpace(url))
                throw new ArgumentException("URL이 비어있습니다.", nameof(url));

            Match match = VideoIdPattern.Match(url);
            if (!match.Success)
                throw new ArgumentException("유효한 유튜브 URL이 아닙니다.", nameof(url));
        ...
        public async Task<VideoMetadata> GetAsync(string normalizedUrl, string toolDirectory, CancellationToken cancellationToken) {
            ...
            if (!File.Exists(ytdlpPath))
                throw new FileNotFoundException("yt-dlp.exe를 찾을 수 없습니다. 설정에서 다운로드해주세요.", ytdlpPath);
