...
public static class GitHubUpdater {
    public static async Task CheckAndUpdateAsync(
        ...
        try {
            ...
            JToken asset = FindWinFormsInstaller(assets, latestVersion);

            if (asset == null) {
                Tol.ShowError(
                    "이 릴리즈에서 WinForms용 설치 파일을 찾지 못했습니다.\n" +
                    $"필요한 파일 이름: GMTFV-{latestVersion}-setup.exe");
                return;
            }
    ...
    private static JToken FindWinFormsInstaller(JArray assets, Version version) {
        if (assets == null) return null;

        string expectedName = $"GMTFV-{version}-setup.exe";
        JToken exactMatch = assets.FirstOrDefault(asset =>
            string.Equals(
                asset["name"]?.ToString(),
                expectedName,
        ...
        // 과거 릴리즈의 이름 변형은 허용하되 WPF Preview 설치 파일은 절대 선택하지 않습니다.
        return assets.FirstOrDefault(asset => {
            string name = asset["name"]?.ToString() ?? string.Empty;
            return name.StartsWith("GMTFV", StringComparison.OrdinalIgnoreCase) &&
                   name.IndexOf("setup", StringComparison.OrdinalIgnoreCase) >= 0 &&
                   name.EndsWith(".exe", StringComparison.OrdinalIgnoreCase) &&
                   name.IndexOf("wpf", StringComparison.OrdinalIgnoreCase) < 0 &&
