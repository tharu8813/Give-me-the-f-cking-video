...
namespace GMTFV.services {
    ...
    internal static class YtDlpDownloadCommandFactory {
        public static YtDlpDownloadCommand Create(
            ...
            // 공급자 준비 시 mweb 고화질 포맷에 필요한 PO Token을 자동 발급합니다.
            // 공급자를 시작할 수 없을 때만 PO Token 불필요 클라이언트로 폴백합니다.
            string youtubeClientOption = YtDlpTool.GetYoutubeClientArguments();
            string retryOption = "--retries 3 --fragment-retries 3 --retry-sleep http:exp=1:5";
            string arguments = $"{ffmpegOption} {youtubeClientOption} {retryOption} {formatOption} {conversionOption} {progressOption} --no-playlist --newline -o \"{outputPath}\" \"{url}\"".Trim();
